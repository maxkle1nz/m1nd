...
flowchart TB
    ...
    subgraph Owner["THE SERVED OWNER — one process, one graph, one memory (default :1338)"]
        ...
        VERBS["Verb surface — 118 tools<br/>server.rs registry · budget law"]
...
15. **Catalog↔dispatch drift:** 5 `lock_*` verbs dispatchable but uncataloged/invisible to help; no parity test pins catalog(118)↔dispatch↔ESSENTIAL. *(tool-surface)*
...
### HONESTY / CLEANUP
21. Extractor trait doc claims all-tree-sitter; five core extractors are regex. *(ingest-light)* · Stale "102 tools"/"~25 curated" literals in four docstrings — **CLOSED** (registry-derived 118/42 now stated). *(tool-surface)* · `query`/`query_readonly` duplicate ~120 lines the doc promises byte-identical. *(graph-core)* · L1GHT supersession is frontmatter-only (no graph edge). *(ingest-light)* · Silent non-convergence in SpectralGapAnalyzer. *(graph-core)*
