...
- **Owner-down guard.** `apply`/`rollback` probe the served-owner loopback port (`127.0.0.1:1338`) and refuse while a listener is up — the offline migration must not race a live keepalive owner; the maintainer stops the served owner first.
