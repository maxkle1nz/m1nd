...
| `graph_changed` | **Something a viewer renders actually mutated** — an agent ran a graph write (`memorize` / `ingest` / `edit_commit` / `apply` / `learn`) OR a Build-Map write (`system_blocks_ratify` / `_reconcile` / `_seed_import` / `_archive` / `_delete`, `skeleton_candidate`, `receipt_import`, `xray_paint` / `xray_retag` — the SystemBlock store, the skeleton and the X-RAY tags the map draws). Carries `{ event, agent_id?, source?, batch_id?, timestamp_ms?, brain_root? }`. The optional **`brain_root`** (§4A.9.6, additive — absent on a bound/legacy mutation and on a pre-2H owner) names WHICH brain mutated when the mutating call used `?brain=`; a viewer refetches when it names ITS brain OR carries no field (the honest over-refetch on old owners). |

`graph_changed` is the browser (pure-reader) counterpart of the MCP transport's `notifications/m1nd/graph_changed` notification (`m1nd-mcp/src/mcp_http.rs`). Both renderings share ONE mutation-detection predicate — `mcp_http::graph_mutation_event_name`, gated by `GRAPH_MUTATION_TOOLS` — so a read-only tool result never masquerades as a graph change on either surface. That set is a **curated subset** of `server::READ_ONLY_DENIED_TOOLS` (every entry is a write, but writes a viewer never draws — mailbox writes like `mission_post`, advisory leases, the activation overlay — are deliberately excluded). On the browser side `http_server::browser_graph_changed_event` derives the `graph_changed` event from a mutation event and the SSE handler emits it alongside the raw event. The served Living Tree **and the Build Map** listen for `graph_changed` to refresh their snapshot in place (debounced ~500 ms, scoped to the brain in view; a low-frequency `/api/graph/stats` poll is the graceful fallback when SSE is unavailable).
