...
### ★ #4 — Stop-Exploring Gate (`stop_gate`) — the sufficiency+coverage arbiter (HONEST REFRAME)

The decision a human never faces and an agent faces most: *am I done exploring, or do I keep pulling context?*

- **Reframe — the token-economics claim is DROPPED.** The original "Round-Trip Solvency" framing promised an *economics arbiter* that answers "will I run dry mid-edit?" from a **remaining-token-budget m1nd knows**. m1nd has **no token-consumption ledger** and cannot source one: `budget_consumed` (`mission_handlers.rs:1020`) is a tool-CALL-count fraction needing an open mission; `focus.token_budget` is an **agent-supplied input**; `focus.budget.used` is a per-call `chars/4` *estimate* (`result_shaping.rs:114`). None measures session token consumption, and building a real one is **host-owned, not m1nd's job**. So this verb is NOT a solvency/economics arbiter — it is an honest **stop-exploring gate** over the sufficiency + coverage signals m1nd genuinely has.
- **Composes (real, dimensionally-coherent signals ONLY):** `focus`'s answer-free `sufficiency` verdict (`sufficient|gathering|saturated`, `layer_handlers.rs:106`) + `focus.ignored.count` (`layer_handlers.rs:918`, can never read 0 under truncation) × `coverage_session` visited/total + `coverage_pct` + unvisited files (`audit_handlers.rs:1039`) × `mission_next.do_not` (the mission-scoped "stop using this tool" list, `mission_handlers.rs:840`). An **OPTIONAL host-supplied `remaining_budget`** enables a *truthful fit-report only* (see below) — it is never treated as consumption.
- **New question:** *"Given what I've loaded, is the strongest context in hand (`enough`), is a strong relevant node still dropped or coverage still thin (`keep_gathering`), is the goal simply not represented here (`not_here`), or am I re-calling a tool the mission already told me to stop (`looping`)?"*
- **Verdict mapping:** `enough` ⇐ `sufficiency.state=="sufficient"` AND `ignored.count==0` (you hold every relevance-clearing node). `keep_gathering` ⇐ `sufficiency.state=="gathering"` (a strong node was dropped) OR low `coverage_pct` with unvisited relevant files. `not_here` ⇐ `sufficiency.state=="saturated"` / empty graph (broaden the goal, ingest more). `looping` ⇐ mission open AND `do_not` non-empty for the tool about to repeat. Honesty degrades, never fabricates: no mission ⇒ the `do_not` signal is reported UNAVAILABLE; empty graph ⇒ `needs_ingest`; no `remaining_budget` ⇒ no fit-report at all.
- **The optional honest budget HALF (fit-report, not a ledger):** when the host passes `remaining_budget` (tokens it declares it has left), the gate reuses `pack_to_budget`/the `chars/4` estimate to report a **truthful fit**: "your declared budget fits N of the M relevance-clearing nodes." That is the ONLY budget claim ever made. **m1nd NEVER claims to know consumed/spent tokens.** Absent `remaining_budget`, the fit-report is omitted entirely — not faked.
- **Why it matters for an agent:** the stop/continue decision made actionable *before* the agent over-answers or spins. Stopping too early ⇒ wrong action; too late ⇒ churn re-confirming what you already hold. The honesty floor holds by construction: `focus.ignored.count` can't read 0 under truncation, so the gate can never be talked into a false `enough`; `mission_next` supplies the anti-loop signal only when a mission is actually open.
- **What was corrected (see §O.5):** the earlier spec's promise to "**wire a true token budget** (or build it net-new)" is **removed** — m1nd cannot source token consumption itself, so that promise was a load-bearing lie. Net-new here is ONLY the verb wiring + verdict mapping + the optional host-supplied fit-report; every signal it fans out already ships.
...
2. **The answer-free sufficiency stop signal** — *"do I have enough to act, or keep gathering?"* — an agent-native question with no human-tool analog (the token-"solvency"/"afford to finish" framing is DROPPED: m1nd has no token ledger — see §O.3 #4).
...
- **Move 2 — Stop-Exploring Gate (`stop_gate`) — HONEST REFRAME.** A "should I stop exploring?" gate over `focus`'s `sufficiency` verdict + `focus.ignored` (can't read 0 under truncation) + `coverage_session` (visited/total, coverage_pct, unvisited) + `mission_next.do_not` (the anti-loop signal, when a mission is open). **The token-economics claim is DROPPED:** m1nd has **no token ledger** and cannot source consumption, so there is no "wire a true token budget" step (that was the load-bearing lie the §O.3 #4 correction removes). Remaining budget is **host-supplied and optional**, used only for a truthful `chars/4` **fit-report** ("your declared budget fits N of M relevance-clearing nodes") — never a consumption claim. Honesty degrades, never fabricates: no mission ⇒ `do_not` unavailable; empty graph ⇒ `needs_ingest`; no `remaining_budget` ⇒ no fit-report.
