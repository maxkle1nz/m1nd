...
- **[medium] No concurrency / lost-update protection**: every handler does load→mutate→`fs::write` (truncate-in-place) with no lock, no CAS, no tmp+rename. Two agents/hook-fires on one `mission_id`, or a crash mid-write, can interleave/lose events or leave half-written JSON that `load_mission` then fails to parse.
...
- **[medium] Evidence classification is lexical**: `is_direct_kind` is substring matching. A caller can mint a `direct` grade by naming an event `file_read` or putting `grep` in an `evidence_ref` without any real read (`classify_evidence` early-returns `direct` from refs alone, :944-949; test `direct_evidence_wins` passes `file_read:src/auth.rs:42` with `events:[]`). The gate trusts the label, not the act.
