...
- **(5c) The gate & the candidate (oracle objection 4):** config names a `gate_command`; runnerd runs it after the agent exits, hashes the full log, and on zero exit emits a `merge_wait` letter carrying a **complete `receipt_candidate`** (§1 — block, type, scope read from a fresh snapshot, evidence with the real artifact hash). **Runnerd holds NO `receipt_import` permission in the MVP**; the import is a human act from the tray (§6, F2.5d) or an explicitly-authorized agent session outside runnerd. Only after a confirmed import may a `landed` letter (1d) be posted.
