...
- **(5e) Out of MVP, declared:** loop-runner/review-runner adapters, parallel mission scheduling, remote runners, automatic receipt import, the Routing Rules screen (per-mission manual runner choice in compose for now). **AMENDED by F12 (2026-07-11): the `hand-runner` refusal is LIFTED for the curation shape ONLY** — a hand-runner serves the synchronous `/curate` propose-apply lane (it PROPOSES `candidate_edit` ops as data; the owner sanitizes and applies them, seat `runner`, under OCC — see HUMAN-VIEW-V2-F12-TECH). `loop`/`review-runner` stay refused byte-identically; `/run` mission spawning of a hand-runner is not a shape (curation is never a worktree mission).
