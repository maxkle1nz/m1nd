...
> **SHIPPED 2026-07-23 (P1 — the medulla-only READ fallback, honest).** The read half of the degradation ladder rung 3 (§10.4). When the caller's root is KNOWN, no project brain covers it, and the served store is the MEDULLA (`SessionState::caller_root_is_brainless` — the SAME triple the write path refuses as `brainless_root`, left untouched), `north` now: **(a)** serves the medulla's cross-project doctrine + promoted memory as a LEGITIMATE feed (the existing tier/origin-labeled memory beat, unchanged — its recall is scoped to the `light::` id namespace, so it structurally cannot carry code); **(b)** CUTS THE POISON — it no longer runs `orient` over the bound graph and hands back its `focus_nodes`/`anchors` as the caller's `context` (the Antigravity-class leak: a foreign graph's code anchors passed off as the caller's own — RED-proven then GREEN); `context` is `null`; **(c)** stamps the canonical `project_brain_absent` symbol (was **doc-only** — now `session.rs::PROJECT_BRAIN_ABSENT`) on the `reception` block (additively) + an `honest_gaps` line; **(d)** suppresses the `needs_ingest` (empty-graph) narrative so a served-medulla beat is never mislabeled "nothing ingested". **Honest divergences from the aspirational rung-3 wording** (`fix: m1nd init` + `needs_ingest`): the next-action is NOT `m1nd init` — that BIRTH is unbuilt (today `m1nd init` is an installer, and the typed bootstrap consumer is not installed), so the read path speaks the SAME closed-bootstrap recovery the write path already ships (`bootstrap_unavailable` / `brain_bootstrap_consumer_not_installed`), never advertising an unreachable birth; and `+ needs_ingest` is deliberately NOT emitted (the honest label is `project_brain_absent`). `reception_verdict` enrichment is strictly ADDITIVE: every field `human_view` (honest/caller_root/bound_workspace) and `mcp_http::enrich_reception_with_roster` (`match` + the `bootstrap_unavailable` option) read is preserved; the `honest` CODE-coverage line stays byte-stable; a genuine project-brain misbind keeps the plain distrust block (no `project_brain_absent`). **P1 is READ-ONLY — still open (P2/P3):** brain BIRTH (`m1nd init` as the create path) and the live `bind` verb; the sovereign write frontier (`brain.bootstrap`, `external_mutation_service`) untouched. **Proof:** RED→GREEN in `m1nd-mcp/src/server.rs` (`north_brainless_caller_serves_medulla_not_foreign_anchors` — the poison leak captured then cut; `north_covered_caller_keeps_grounded_context` — no regression) + `m1nd-mcp/src/session.rs` (`reception_verdict_medulla_mismatch_labels_project_brain_absent_additively`, `reception_verdict_project_brain_mismatch_stays_a_plain_distrust_block`, `reception_verdict_covered_caller_is_silent`); full `cargo test -p m1nd-mcp` green (the 3 known actor/timing flakes pass in isolation), `cargo clippy --workspace -- -D warnings` + `cargo fmt --check` clean.
...
3. Project brain absent → medulla-only + `gaps:[{kind: project_brain_absent, fix: "m1nd init"}]` + `needs_ingest`. *(**SHIPPED 2026-07-23, P1 read half — §9.5.5 note, with honest divergences:** the `project_brain_absent` gap + the poison cut are live in `north`; the `fix` is NOT `m1nd init` (that birth is unbuilt — the read path speaks the write path's closed-bootstrap recovery instead) and `needs_ingest` is NOT emitted (the honest label is `project_brain_absent`, not an unfinished ingest).)*
