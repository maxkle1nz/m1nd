...
La superficie MCP live si evolve con i rilasci — usa `tools/list` per il conteggio esatto degli strumenti e i nomi nella tua build. **Menu centrale:** di default vengono pubblicizzati circa 15 strumenti — il nucleo ratificato dal proprietario più la base di binding dell'host — perché sei settimane di traffico misurato hanno mostrato 141 strumenti pubblicizzati e 13 realmente chiamati. Imposta `M1ND_TOOL_TIER=full` per pubblicizzare il registro completo (140+ strumenti: RETROBUILDER, perspectives, federation, daemon). Nulla viene rimosso: gli strumenti nascosti restano chiamabili per nome via `tools/call`, e `help` cataloga e spiega TUTTO il registro a qualsiasi livello — è la porta. Il catalogo strumento per strumento non vive in questo README: vedi il [wiki canonico](https://m1nd.world/wiki/), [docs/AGENT-PACKS.md](../docs/AGENT-PACKS.md) e [EXAMPLES.md](../EXAMPLES.md) per la profondità, e [CHANGELOG.md](../CHANGELOG.md) per la cronologia dei rilasci.
