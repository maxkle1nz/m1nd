            ...
            "function_item" | "struct_item" | "enum_item" | "trait_item" | "type_item" => {
                if let Some(name) = Self::semantic_extract_name(node, source) {
                    ...
                    if node.kind() == "function_item" {
                        // The tree-sitter node knows the item's REAL end row —
                        // record it so the graph's provenance carries the true
                        // extent instead of line_start == line_end. The impl_item
                        // arm above has always done this for impl blocks.
