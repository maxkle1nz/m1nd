...
//! exact seam attach bridges hit — so the audited crossing is proven end-to-end,
//! not unit-mocked. The pure logic (gate, hygiene floor, re-anchor, parse) is
//! unit-tested in `promote_handlers::tests`; this file pins the CROSSING.
//!
//! THE LAWS UNDER TEST (MEDULLA-PRD §7, ORGANISM-PRD §C8.2/§C8.3):
//!   - promote copies a VERIFIED project claim UP into the medulla with the full
...
//!   - C8.3: only State: verified (or founder-sourced) may promote; an unverified
//!     maker claim is refused INSIDE the verb.
...
//!
//! RED before M6: no promotion surface existed at all (grep-proven) — a
//! cross-project finding could not reach another brain by ANY recorded act. This
//! file is the GREEN that could not exist before the verb.
#![cfg(feature = "serve")]
