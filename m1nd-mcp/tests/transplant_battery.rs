...
// proof-grown loop. Contract source: the donor study behind
// docs/TRANSPLANT-PRD.md — a 7-phase spec composed from rope (region widening,
// placeholder rename, over-provision→prune imports), ts-morph (dependency
// trichotomy, canonical phase order), Roslyn (dest-collision preflight) and
// recast (indent + trivia ownership).
//
// Canonical scenario:
...
//
// NOTE on addressing: `symbol` + `source_file` is the canonical v1 form
// (PRD §10, decision D1); what this battery pins is the OBSERVABLE on-disk
