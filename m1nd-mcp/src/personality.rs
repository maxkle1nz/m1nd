...
pub fn tool_docs() -> Vec<ToolDoc> {
    vec![
        ...
        ToolDoc {
            ...
            category: "Foundation",
            glyph: GLYPH_DIMENSION,
            one_liner: "Server health -- graph size, uptime, sessions, host-binding contract",
            params: &[("agent_id", "Calling agent identifier", true)],
            returns: "Status, node/edge counts, uptime, active sessions, binding fingerprint, and tool-surface contract",
            example: r#"{"agent_id": "jimi"}"#,
            next: &["trust_selftest", "session_handshake", "doctor"],
        },
        ToolDoc {
            name: "trust_selftest",
            category: "Foundation",
            glyph: GLYPH_DIMENSION,
            one_liner: "Trust selftest -- one-call verdict for m1nd binding and recovery readiness",
            params: &[
                ("agent_id", "Calling agent identifier", true),
                ("observed_tool_count", "Optional host tools/list count", false),
                ("available_tools", "Optional host-exposed tool names", false),
                ("observed_tool", "Tool that produced a suspicious result", false),
                ...
                ("scope", "Optional repo/scope path for Context Guard binding checks", false),
            ],
            returns: "Verdict, checks, binding fingerprint, graph state, context_guard, session handshake, and optional recovery playbook",
            example: r#"{"agent_id": "jimi"}"#,
            next: &["session_handshake", "recovery_playbook", "ingest"],
        },
        ToolDoc {
            ...
            returns: "Trust mode, capability flags, graph state, tool-surface status, context_guard, and doctor recovery payload",
            example: r#"{"agent_id": "jimi"}"#,
            next: &["recovery_playbook", "ingest", "doctor"],
        },
        ToolDoc {
            name: "recovery_playbook",
            category: "Foundation",
            glyph: GLYPH_DIMENSION,
            one_liner: "Recovery playbook -- deterministic next steps for degraded or stale sessions",
            params: &[
                ("agent_id", "Calling agent identifier", true),
                ("trust_mode", "Optional prior handshake trust mode", false),
                ("observed_tool", "Tool that produced a suspicious result", false),
                ("observed_proof_state", "Observed proof_state, such as blocked", false),
                ("observed_candidates", "Observed retrieval candidate count", false),
                ("scope", "Optional repo/scope path for Context Guard binding checks", false),
            ],
            returns: "Status, trust mode, context_guard, binding fingerprint, graph/tool summaries, and ordered recovery steps",
            example: r#"{"agent_id": "jimi", "observed_tool": "seek", "observed_proof_state": "blocked", "observed_candidates": 0}"#,
            next: &["doctor", "session_handshake", "ingest"],
        },
        ToolDoc {
            name: "doctor",
            category: "Foundation",
            ...
            returns: "Graph/runtime/session state, context_guard, warnings, probable causes, next actions",
            example: r#"{"agent_id": "jimi", "observed_tool": "seek", "observed_proof_state": "blocked", "observed_candidates": 0}"#,
        ...
        // Brand gate G1.5: the `savings` ToolDoc was removed with the tool.
        ToolDoc {
            name: "report",
            category: "Report",
            glyph: GLYPH_DIMENSION,
            one_liner: "Session intelligence report -- queries, elapsed, heuristic hotspots",
            params: &[("agent_id", "Calling agent identifier", true)],
            returns: "Markdown report with query log, graph size, and heuristic hotspots",
            example: r#"{"agent_id": "jimi"}"#,
            next: &["audit", "trail_save"],
        },
        ToolDoc {
            name: "audit",
            category: "Report",
        ...
        ToolDoc {
            ...
            category: "Mission",
            glyph: GLYPH_STRUCTURE,
            one_liner: "Start a bounded agent mission with route, budget, and proof non-claims",
            params: &[
                ("agent_id", "Calling agent identifier", true),
                ("repo", "Repository path this mission is scoped to", true),
                ("task", "Mission task in plain language", true),
                ("mode", "bug_hunt | review | refactor | docs_drift | architecture | release", false),
                ("budget", "short | normal | deep", false),
                ("risk", "low | medium | high", false),
            ],
            returns: "Mission id, route, trust projection, budget envelope, starter moves, and non-claims",
            example: r#"{"agent_id": "jimi", "repo": "/project", "task": "audit auth/session boundary", "mode": "review", "budget": "short", "risk": "medium"}"#,
            next: &["mission_event", "mission_next", "search", "audit"],
        },
        ToolDoc {
            name: "mission_event",
            category: "Mission",
            glyph: GLYPH_CONNECTION,
            one_liner: "Record one observed mission action with evidence class and digest",
            params: &[
                ("agent_id", "Calling agent identifier", true),
                ("mission_id", "Mission id returned by mission_start", true),
                ("event", "Observed action object such as file_read, test_run, graph_query, dissent, or coverage_sweep", true),
            ],
            returns: "Event id, normalized event, evidence class, event count, digest, and non-claims",
            example: r#"{"agent_id": "jimi", "mission_id": "msn_123_jimi", "event": {"event": "file_read", "path": "src/auth.rs", "outcome": "supports claim"}}"#,
            next: &["mission_next", "mission_verify", "mission_handoff"],
        },
        ToolDoc {
            name: "mission_next",
            category: "Mission",
            glyph: GLYPH_PATH,
            one_liner: "Advance a mission by one move and emit do-not guardrails",
            params: &[
                ("agent_id", "Calling agent identifier", true),
                ("mission_id", "Mission id returned by mission_start", true),
                ("last_event", "Optional event from the action just taken", false),
            ],
            returns: "One next move, phase, do-not list, budget use, and dissent protocol",
            example: r#"{"agent_id": "jimi", "mission_id": "msn_123_jimi", "last_event": {"event": "graph_query", "tool": "seek", "outcome": "inconclusive"}}"#,
            next: &["mission_event", "mission_verify", "mission_handoff", "mission_close"],
        },
        ToolDoc {
            name: "mission_verify",
            category: "Mission",
            glyph: GLYPH_CONNECTION,
            one_liner: "Check whether a mission claim has direct proof instead of graph-only evidence",
            params: &[
                ("agent_id", "Calling agent identifier", true),
                ("mission_id", "Mission id returned by mission_start", true),
                ("claim", "Candidate conclusion to validate", true),
                ("evidence_refs", "Direct proof refs such as file_read:path:line or test_run:name", false),
                ("confidence", "Optional agent confidence before verification", false),
            ],
            returns: "Verdict, evidence grade, missing proof, and next required move",
            example: r#"{"agent_id": "jimi", "mission_id": "msn_123_jimi", "claim": "logout clears session", "evidence_refs": ["file_read:src/auth.rs:42"]}"#,
            next: &["mission_next", "mission_handoff", "mission_close"],
        },
        ToolDoc {
            name: "mission_handoff",
            category: "Mission",
            glyph: GLYPH_STRUCTURE,
            one_liner: "Serialize a resumable mission packet for another agent or later session",
            params: &[
                ("agent_id", "Calling agent identifier", true),
                ("mission_id", "Mission id returned by mission_start", true),
                ("summary", "Optional handoff summary", false),
                ("recipient_agent_id", "Optional recipient agent id", false),
                ("include_events", "Include full event stream in the packet", false),
            ],
            returns: "Handoff packet with verified claims, open hypotheses, dead paths, graph anchors, next move, digest, gaps, and non-claims",
            example: r#"{"agent_id": "jimi", "mission_id": "msn_123_jimi", "summary": "resume at direct proof", "recipient_agent_id": "reviewer"}"#,
            next: &["mission_next", "mission_close"],
        },
        ToolDoc {
            name: "mission_close",
            category: "Mission",
            glyph: GLYPH_STRUCTURE,
            one_liner: "Close a mission with a proof packet, gaps, rejected claims, and non-claims",
            params: &[
                ("agent_id", "Calling agent identifier", true),
                ("mission_id", "Mission id returned by mission_start", true),
                ("summary", "Optional concise mission summary", false),
                ("non_claims", "Extra non-claims to preserve", false),
                ("gaps", "Known remaining gaps", false),
            ],
            returns: "Proof packet with verified claims, rejected claims, events, budget, gaps, and non-claims",
            example: r#"{"agent_id": "jimi", "mission_id": "msn_123_jimi", "summary": "verified logout boundary"}"#,
            next: &["report", "trail_save"],
        },
        ToolDoc {
            name: "view",
            category: "Surgical",
