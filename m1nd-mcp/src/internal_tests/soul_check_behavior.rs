...
fn build_repo(tag: &str) -> (PathBuf, SessionState) {
    ...
    // Keep the Cargo fixture valid for the code ingestor's metadata pass.
    // A package manifest without lib/bin targets is rejected by cargo before
    // any SOUL assertion can run.
    write(&repo.join("src/lib.rs"), "pub mod widget;\n");
