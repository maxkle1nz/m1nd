...
//! body of `GET /api/brains`) against a REAL owner. Read traffic uses the SAME
//! wire seam (`handle_mcp_post`) the two-tier tests use; sovereign fixture birth
//! runs through the owner's actor instead of reopening generic ingest:
...
impl Owner {
    ...
    /// Seed the bound graph through the owner actor. Generic `ingest` is
    /// sovereign-frozen until its exact typed consumer exists, so fixture setup
    /// must not pretend the public generic door is authorized.
    ...
    /// Run the production bootstrap core through the owner actor and bind the
    /// fixture wire session exactly as the transport seam would after approval.
    ...
    /// Read bound-brain live counters through its actor; once the actor owns the
    /// SessionState, a raw `session.lock()` is an invalid second owner.
    fn bound_live_stats(&self) -> (u64, u64) {
        self.app
            .project_brains
            .read_target_runtime_snapshot(Arc::clone(&self.app.session), None, true, |state| {
                Ok((state.sessions.len() as u64, state.queries_processed))
            })
            .expect("bound live-stats snapshot")
            .value
    }
}
...
/// brain bootstrapped through the production owner core.
async fn owner_with_two_brains(tmp: &Path) -> (Owner, PathBuf, PathBuf) {
    ...
    // Preserve the live per-brain presence semantics the Hall reports. This is
    // an ordinary read through the real wire after actor-authorized birth.
    owner
        .tool(
            &sid,
            &bound_repo,
            "north",
            serde_json::json!({"task": "fixture orientation", "agent_id": "setup"}),
        )
        .await;

    // Hosted project brain via the production bootstrap core (bound graph intact).
    let project_repo = tmp.join("project-repo");
    write_project_repo(&project_repo);
    let (sid_p, boot) = owner
        .bootstrap_actor(&project_repo, "project-b-agent")
        .await;
    assert_eq!(
        boot["schema"], "m1nd-project-brain-bootstrap-v0",
        "the project brain must be born through the production bootstrap core: {boot}"
    );
    owner
        .tool(
            &sid_p,
...
async fn per_brain_counters_partition_on_the_bound_brain_not_owner_global() {
    ...
    let (owner_wide_sessions, bound_own_q) = owner.bound_live_stats();
...
async fn capture_brains_fixture() {
    ...
    let sid = owner.init_session(&bound_repo).await;
    owner.ingest_bound_actor(&bound_repo, "setup");
    owner
        .tool(
            &sid,
            &bound_repo,
    ...
    let (sid_c, _boot) = owner.bootstrap_actor(&cherry, "project-b-agent").await;
    owner
        .tool(
            &sid_c,
            &cherry,
