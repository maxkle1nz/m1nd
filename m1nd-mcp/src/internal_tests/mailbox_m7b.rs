...
//! (seeded through the owner actor; generic mutation remains authority-frozen), then the box
...
impl Owner {
    ...
    /// Bind this owner's graph through its actor. Generic `ingest` remains
    /// sovereign-frozen until the typed consumer exists.
    async fn bind_to(&self, repo: &Path) {
        let _sid = self.init(repo).await;
...
fn mailbox_get(owner: &Owner, brain: Option<&str>, foreign: &BTreeSet<String>) -> Value {
    ...
    let (_session, served) = resolve_brain(&owner.app, brain).unwrap();
    // `resolve_brain` already returns the actor-safe served identity. Reading the
    // raw SessionState here would attempt to become a second owner after actor
    // startup and is deliberately forbidden.
    let repo_root = served["project_root"]
        .as_str()
        .expect("resolved brain must echo project_root")
        .to_string();
