...
fn graph_search_node_id(state: &mut SessionState, query: &str) -> String {
    ...
    // Literal search intentionally ranks a matching on-disk FileContent row
    // ahead of graph ids. Perspective focus needs the graph node, so select it
    // by the external-id contract instead of depending on result order.
    .and_then(|results| {
        results.iter().find(|entry| {
            entry
                .get("node_id")
                .and_then(|value| value.as_str())
                .is_some_and(|node_id| node_id.to_ascii_lowercase().contains(&needle))
        })
    })
...
fn perspective_start_list_close_lifecycle_reflects_inputs() {
    ...
    let ingested_node_id = graph_search_node_id(&mut state, "AlphaNode");
    assert!(
