...
// ---------------------------------------------------------------------------
// (3) VITALS NEVER BLOCK THE PANORAMA — the sidecar-only surface must not touch
// SessionState at all. A legacy pre-actor guard held for seconds cannot delay
// the response or change the owner count: the daemon-alert sidecar is the sole
// source. Law: F30 §3a, vitals never block the panorama.
// ---------------------------------------------------------------------------

#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn universe_is_sidecar_only_while_a_legacy_session_guard_is_held() {
    let tmp = tempfile::tempdir().unwrap();
    let runtime = tmp.path().join("runtime");
    let app = mk_app(&runtime);
    ...
    // Missing canonical sidecar is the defined empty state, independent of the
    // in-memory owner guard. No busy-state note is inferred from a mutex probe.
    assert_eq!(
        body["owner"]["alerts_pending"],
        serde_json::json!(0u64),
        "missing alert sidecar is the canonical empty owner scope: {body}"
    );
    assert!(body["owner"].get("note").is_none());
