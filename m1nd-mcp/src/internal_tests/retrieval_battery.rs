...
//! BEFORE any fix. Each case is NUMBERED and drives reads through the REAL
//! Streamable-HTTP seam (`handle_mcp_post`) in-process — the exact door the
//! attach bridges hit. Sovereign fixture setup runs through the owner actor,
//! because generic REST/MCP bootstrap and federation are intentionally
//! fail-closed until their exact typed authority consumers exist. Every case
...
impl Owner {
    ...
    /// Explicitly authorized fixture ingest through the selected project actor.
    /// The public generic ingest door stays sovereign-frozen; this is setup for
    /// retrieval assertions, not a transport authorization claim.
    fn ingest_project_actor(&self, root: &Path, arguments: Value) -> Value {
        let key = ProjectBrainRegistry::canonical_key(&root.to_string_lossy());
        let brain = self
            .app
            .project_brains
            .try_resolve(&key)
            .expect("resolve project brain for fixture ingest")
            .expect("fixture project brain must be hosted");
        let input: m1nd_mcp::protocol::IngestInput =
            serde_json::from_value(arguments).expect("fixture ingest input");
        self.app
            .project_brains
            .execute_target_m1nd(brain, Some(&key), false, true, move |state| {
                m1nd_mcp::tools::handle_ingest(state, input)
            })
            .expect("project fixture ingest actor")
    }

    /// Run sovereign federation through the bound owner actor. If it crosses
    /// the measurement deadline, keep awaiting the same task to terminal state
    /// before reporting failure so the test never detaches a late mutation.
    async fn federate_bound_actor(
        &self,
        arguments: Value,
        ...
        let mut task = tokio::task::spawn_blocking(move || {
            app.project_brains.execute_target_m1nd(
                ...
                true,
                true,
                move |state| m1nd_mcp::server::dispatch_tool(state, "federate", &arguments),
            )
        });
        let (within_deadline, joined) = match tokio::time::timeout(deadline, &mut task).await {
            Ok(joined) => (true, joined),
            Err(_) => (false, task.await),
        };
        let terminal = joined
            .map_err(|error| format!("federate actor task failed: {error}"))
            .and_then(|result| result.map_err(|error| error.to_string()));
        (within_deadline, terminal)
    }
...
async fn c3_cross_verify_light_not_missing() {
    ...
    let ing = owner.ingest_project_actor(
        &root,
        serde_json::json!({
            "path": mem_dir,
            "adapter": "light",
...
async fn c5_federate_clock() {
    ...
    async fn run(files: usize, per_file: usize, tag: &str) -> (bool, f64, u64, bool, f64, usize) {
        ...
        let t0 = Instant::now();
        let (fed_within_deadline, fed) = owner
            .federate_bound_actor(fed_args, Duration::from_secs(90))
            .await;
        let fed_ms = t0.elapsed().as_secs_f64() * 1000.0;
        let (fed_ok, nodes) = match fed {
            Ok(v) if fed_within_deadline => (
                true,
                v["total_nodes"]
                    .as_u64()
            ...
            Ok(_) => (false, 0),
            Err(error) => {
                eprintln!("C5 federate actor error: {error}");
                (false, 0)
            }
        };
