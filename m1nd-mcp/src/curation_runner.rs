...
mod tests {
    ...
    fn curate_candidate_happy_path_applies_seat_runner_and_posts_the_summary() {
        ...
        .to_string();
        let (port, _d) = spawn_fake_daemon("HTTP/1.1 200 OK", body, TEST_RUNNER_SECRET);
    ...
    #[test]
    fn curate_response_parser_refuses_oversized_or_incomplete_bodies() {
        assert!(parse_curate_response(&vec![b'x'; 2 * 1024 * 1024 + 1])
            .unwrap_err()
            .contains("exceeded"));
        let incomplete = b"HTTP/1.1 200 OK\r\nContent-Length: 100\r\n\r\n{}";
        assert!(parse_curate_response(incomplete)
            .unwrap_err()
            .contains("incomplete"));
    }
}
