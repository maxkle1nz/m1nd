...
//!
//! The first verb where the GRAPH WRITES: the caller names a symbol and two
//! paths, the server computes the whole move. Design and proof addresses:
//! `docs/TRANSPLANT-PRD.md`. The verb:
...
//!   3. Widens the item region by PARSING (tree-sitter), carrying doc comments,
...
//!
//! Scope is deliberately narrow (top-level `fn` items in Rust — PRD §7 states
//! the seven v1 boundaries); everything the graph could not answer confidently
//! is surfaced honestly in [`TransplantOutput`] (`refs_unresolved`,
//! `dependency_source`, `referencer_source`, `state_left_behind`) rather than
//! silently skipped. The receipt never promises what the parser cannot see.
...
fn plan_transplant(state: &SessionState, input: &TransplantInput) -> M1ndResult<TransplantPlan> {
    ...
    // v1 requires an existing destination — creating a new module + wiring
    // `mod <dest>;` into lib.rs is a feature of its own (PRD §7.3); refusing is
    // the honest boundary. Zero writes on this path.
    let dest_text = std::fs::read_to_string(&dest_abs).map_err(|_| {
        refuse(format!(
            "dest_file '{dest_abs}' does not exist or cannot be read; transplant requires the destination module to already exist (creating + wiring a new module is out of scope for v1)"
        ))
    })?;
    ...
    if !ts_fns.is_empty() && !ts_is_top_level(&ts_fns, &symbol) {
        return Err(refuse(format!(
            "symbol `{symbol}` is not a TOP-LEVEL `fn` in '{source_abs}' (it is nested in a module, a method, or a closure); transplant v1 moves only top-level free functions"
        )));
    }
...
/// verb refuses a homonym of any kind — a conservative over-refusal that makes the
/// E0428-after-success "false ideal" unreachable and teaches the caller exactly what
        ...
        if body.matches('{').count() > 1 {
            if body.contains(src_mod) && body.contains(symbol) {
                unresolved.push(format!(
                    "{file_label}:{}: nested-group import `{}` mentions `{src_mod}`/`{symbol}` — split it manually, transplant does not rewrite nested groups (PRD §7.4)",
                    st.start + 1,
                    st.text.trim()
                ));
            }
