...
const ANTIBODY_PERSISTENCE_VERSION: u32 = 1;

fn validate_antibody_persistence(antibodies: &[Antibody]) -> M1ndResult<()> {
    if antibodies.len() > MAX_ANTIBODIES {
        return Err(M1ndError::CorruptState {
            reason: format!(
                "antibody registry contains {} entries (limit {MAX_ANTIBODIES})",
                antibodies.len()
            ),
        });
    }
    let mut ids = HashSet::with_capacity(antibodies.len());
    for antibody in antibodies {
        if antibody.id.trim().is_empty() || !ids.insert(antibody.id.as_str()) {
            return Err(M1ndError::CorruptState {
                reason: format!(
                    "antibody registry contains an empty or duplicate id '{}'",
                    antibody.id
                ),
            });
        }
        if !antibody.created_at.is_finite()
            || antibody
                .last_match_at
                .is_some_and(|timestamp| !timestamp.is_finite())
            || !antibody.specificity.is_finite()
        {
            return Err(M1ndError::CorruptState {
                reason: format!(
                    "antibody '{}' contains non-finite persistence fields",
                    antibody.id
                ),
            });
        }
        if antibody.created_at < 0.0
            || antibody
                .last_match_at
                .is_some_and(|timestamp| timestamp < antibody.created_at)
            || !(0.0..=1.0).contains(&antibody.specificity)
        {
            return Err(M1ndError::CorruptState {
                reason: format!(
                    "antibody '{}' contains an invalid timestamp or specificity",
                    antibody.id
                ),
            });
        }
        let node_count = antibody.pattern.nodes.len();
        if node_count > MAX_PATTERN_NODES
            || antibody.pattern.edges.len() > MAX_PATTERN_EDGES
            || antibody.pattern.negative_edges.len() > MAX_PATTERN_EDGES
        {
            return Err(M1ndError::CorruptState {
                reason: format!(
                    "antibody '{}' exceeds persistence pattern limits",
                    antibody.id
                ),
            });
        }
        for edge in antibody
            .pattern
            .edges
            .iter()
            .chain(antibody.pattern.negative_edges.iter())
        {
            if edge.source_idx >= node_count || edge.target_idx >= node_count {
                return Err(M1ndError::CorruptState {
                    reason: format!(
                        "antibody '{}' edge {}->{} escapes its {} pattern nodes",
                        antibody.id, edge.source_idx, edge.target_idx, node_count
                    ),
                });
            }
        }
...
/// Encode the versioned antibody registry as deterministic pretty JSON bytes.
///
/// The input order is preserved because it is part of registry semantics. All
/// floating-point fields are checked before serialization so a non-finite value
/// can never be converted into an ambiguous JSON representation.
pub fn encode_antibodies_json(antibodies: &[Antibody]) -> M1ndResult<Vec<u8>> {
    validate_antibody_persistence(antibodies)?;
    let data = AntibodyPersistence {
        version: ANTIBODY_PERSISTENCE_VERSION,
        antibodies: antibodies.to_vec(),
    };
    serde_json::to_vec_pretty(&data)
        .map_err(|error| M1ndError::PersistenceFailed(format!("antibody serialization: {error}")))
}

/// Decode versioned antibody JSON bytes with strict schema-version and numeric
/// validation. Unlike [`load_antibodies`], this in-memory boundary never falls
/// back to a backup or an empty registry.
pub fn decode_antibodies_json(bytes: &[u8]) -> M1ndResult<Vec<Antibody>> {
    let data: AntibodyPersistence = serde_json::from_slice(bytes).map_err(M1ndError::Serde)?;
    if data.version != ANTIBODY_PERSISTENCE_VERSION {
        return Err(M1ndError::CorruptState {
            reason: format!("unsupported antibody persistence version {}", data.version),
        });
    }
    validate_antibody_persistence(&data.antibodies)?;
    let projected = encode_antibodies_json(&data.antibodies)?;
    if serde_json::from_slice::<serde_json::Value>(bytes)?
        != serde_json::from_slice::<serde_json::Value>(&projected)?
    {
        return Err(M1ndError::CorruptState {
            reason: "antibody checkpoint is not the complete current schema".into(),
        });
    }
    Ok(data.antibodies)
}
...
pub fn save_antibodies(antibodies: &[Antibody], path: &Path) -> M1ndResult<()> {
    let json = encode_antibodies_json(antibodies)?;
...
pub fn load_antibodies(path: &Path) -> M1ndResult<Vec<Antibody>> {
    ...
    match decode_antibodies_json(content.as_bytes()) {
        Ok(antibodies) => Ok(antibodies),
        Err(e) => {
            eprintln!(
            ...
            if bak_path.exists() {
                ...
                match decode_antibodies_json(bak_content.as_bytes()) {
                    Ok(antibodies) => Ok(antibodies),
                    Err(_) => {
...
mod tests {
    ...
    #[test]
    fn antibody_memory_codec_is_deterministic_and_matches_save() {
        let antibodies = vec![make_antibody(AntibodyPattern {
            nodes: Vec::new(),
            edges: Vec::new(),
            negative_edges: Vec::new(),
        })];
        let first = encode_antibodies_json(&antibodies).expect("encode");
        let second = encode_antibodies_json(&antibodies).expect("repeat encode");
        assert_eq!(first, second);

        let dir = tempfile::tempdir().expect("tempdir");
        let path = dir.path().join("antibodies.json");
        save_antibodies(&antibodies, &path).expect("save");
        assert_eq!(std::fs::read(path).expect("saved bytes"), first);

        let decoded = decode_antibodies_json(&first).expect("decode");
        assert_eq!(decoded.len(), 1);
        assert_eq!(decoded[0].id, antibodies[0].id);
    }

    #[test]
    fn antibody_memory_codec_rejects_corruption_version_and_nonfinite() {
        assert!(decode_antibodies_json(b"{").is_err());
        assert!(decode_antibodies_json(br#"{"version":2,"antibodies":[]}"#).is_err());

        let mut antibody = make_antibody(AntibodyPattern {
            nodes: Vec::new(),
            edges: Vec::new(),
            negative_edges: Vec::new(),
        });
        antibody.specificity = f32::NAN;
        assert!(encode_antibodies_json(&[antibody]).is_err());
    }

    #[test]
    fn antibody_checkpoint_codec_rejects_unsafe_pattern_identity_and_shape() {
        let mut out_of_bounds = make_antibody(AntibodyPattern {
            nodes: vec![PatternNode {
                role: "only".into(),
                node_type: None,
                required_tags: Vec::new(),
        ...
        assert!(encode_antibodies_json(&[out_of_bounds.clone()]).is_err());

        out_of_bounds.pattern.edges.clear();
        out_of_bounds.specificity = 1.1;
        assert!(encode_antibodies_json(&[out_of_bounds.clone()]).is_err());

        out_of_bounds.specificity = 0.5;
        assert!(encode_antibodies_json(&[out_of_bounds.clone(), out_of_bounds]).is_err());

        let mut unknown = serde_json::to_value(AntibodyPersistence {
            version: ANTIBODY_PERSISTENCE_VERSION,
            antibodies: vec![make_antibody(AntibodyPattern {
                nodes: Vec::new(),
                edges: Vec::new(),
                negative_edges: Vec::new(),
            })],
        ...
        .expect("value");
        unknown["antibodies"][0]["future_field"] = serde_json::json!(true);
        assert!(
            decode_antibodies_json(&serde_json::to_vec_pretty(&unknown).expect("json")).is_err()
        );

        let mut defaulted: serde_json::Value = serde_json::from_slice(
            &encode_antibodies_json(&[make_antibody(AntibodyPattern {
                nodes: Vec::new(),
                edges: Vec::new(),
                negative_edges: Vec::new(),
            })])
        ...
        .expect("value");
        defaulted["antibodies"][0]["pattern"]
            .as_object_mut()
            .expect("pattern object")
            .remove("negative_edges");
        assert!(
            decode_antibodies_json(&serde_json::to_vec_pretty(&defaulted).expect("json")).is_err()
        );
    }
